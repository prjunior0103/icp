...
import { FileText, Search, Download, Users, BarChart3, GitBranch, UserCheck, ChevronDown, ChevronUp, UserCog, Columns } from "lucide-react";
...
const ABAS: { id: AbaId; label: string; icon: React.ReactNode }[] = [
  ...
  { id: "calibracao",  label: "Calibração",       icon: <Columns size={14}/> },
];
...
function RelatGestor({ atribuicoes, notasMap }: { atribuicoes: Atribuicao[]; notasMap: Map<number, number>; }) {
  ...
  const subordinados = colaboradores.filter(c => filtroGestor ? String(c.gestorId) === filtroGestor : !gestoresIds.includes(c.id));
    ...
    <div className="space-y-3">
      ...
      {subordinados.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-10 text-center text-gray-400">
          <UserCog size={36} className="mx-auto mb-2 text-gray-300"/>Selecione um gestor para ver o painel de equipe
        </div>
      ) : (
        <div className="bg-white rounded-xl border border-gray-200 overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {["Colaborador","Matrícula","Cargo","Área","Resultado","Prêmio Projetado"].map(h => (
                  <th key={h} className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
...
// ─── R6: Calibração ──────────────────────────────────────
function RelatCalibracao({ atribuicoes, notasMap, atribuicoesAll }: { atribuicoes: Atribuicao[]; notasMap: Map<number, number>; atribuicoesAll: Atribuicao[] }) {
  const colabsMap = new Map<number, Colaborador>();
  for (const a of atribuicoes) colabsMap.set(a.colaboradorId, a.colaborador);
  const colaboradores = Array.from(colabsMap.values());
  ...
  function calcResultado(id: number) {
    let r = 0;
    for (const at of atribuicoesAll.filter(a => a.colaboradorId === id)) {
      for (const ig of at.agrupamento.indicadores) {
        r += calcMID(notasMap.get(ig.indicadorId) ?? 0, ig.peso);
      }
    }
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-3">
        ...
        <p className="text-xs text-gray-400 self-center">Mostrando top 5 por padrão. Selecione colaboradores para comparar.</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {colabsFiltrados.map(c => (
          <button key={c.id}
            ...
            className={`text-xs px-2 py-1 rounded-full border transition-colors ${selecionados.includes(c.id) ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-600 border-gray-300 hover:border-blue-400"}`}>
            {c.nome}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              <th className="text-left px-4 py-2.5 text-xs font-semibold text-gray-500 uppercase">Info</th>
              {comparar.map(id => {
                const c = colabsMap.get(id);
                return <th key={id} className="text-center px-4 py-2.5 text-xs font-semibold text-gray-700 whitespace-nowrap">{c?.nome ?? id}</th>;
              })}
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
...
export default function RelatoriosPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {aba === "colaborador" && <RelatColaborador atribuicoes={atribuicoes} notasMap={notasMap}/>}
      ...
      {aba === "calibracao"  && <RelatCalibracao atribuicoes={atribuicoes} notasMap={notasMap} atribuicoesAll={atribuicoes}/>}
    </div>
  );
}
