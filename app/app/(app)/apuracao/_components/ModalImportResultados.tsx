...
export function ModalImportResultados({ cicloId, onDone, onClose }: {
  cicloId: number; onDone: () => void; onClose: () => void;
}) {
  ...
  async function importar() {
    ...
    const res = await fetch("/api/resultados/import", { method: "POST", body: fd });
    const data = await res.json();
    setResultado(data);
    setEnviando(false);
    if (data.criados > 0 || data.atualizados > 0) onDone();
  }

  return (
    <ModalWrapper title="Importar Orçado + Realizado" onClose={onClose} size="md">
      {!resultado ? (
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 bg-blue-700 hover:bg-blue-800 disabled:bg-blue-400 text-white text-sm py-2 rounded-lg transition-colors"
            >
        ...
        <div className="space-y-4">
          ...
          <button onClick={onClose} className="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm py-2 rounded-lg">
            Fechar
          </button>
        </div>
      )}
