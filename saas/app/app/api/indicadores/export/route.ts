...
export async function GET(req: Request) {
  ...
  const indicadores = await prisma.indicador.findMany({ where: { cicloId: Number(cicloId) } });
  ...
  const rows = indicadores.map(i => headers.map(h => (i as Record<string, unknown>)[h] ?? ""));
  ...
  return new NextResponse(buf, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="indicadores_ciclo${cicloId}.xlsx"`,
    },
  });
}
